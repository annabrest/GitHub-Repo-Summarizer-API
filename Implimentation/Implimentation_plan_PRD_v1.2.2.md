...
* Accept full URL
* Extract owner/repo using urlparse
...
* Temporary endpoint `/debug/tree?owner=psf&repo=requests` returns:
...
**Goal:** Pick "most informative files" using a Filter → Prioritize → Cover → Fill strategy.

**Do:** in `app/selection.py` (see `Implimentation/Implimentation_Step8.md` for full detail)

Add constants:
* `MAX_FILES = 25`, `MAX_FILE_BYTES_HARD = 200_000`
* `PRIORITY_PATTERNS`, `LOCK_FILES`, `CODE_EXTENSIONS`, `IMPORTANT_NAMES`, `IMPORTANT_DIRS`, `MANIFEST_FILES`

Implement:
* `is_too_large(size)` — hard cutoff >200KB
* `is_priority(path)` — matches README*, LICENSE, CHANGELOG*, manifests, CI/ops
* `score_file(path, size)` — boosts for entry-point names/dirs/extensions; penalties for lock files, test files, extreme nesting, large size
* `get_package_roots(tree_entries)` — finds dirs with manifest files (monorepo support)
* `select_files(tree_entries) -> list[str]`:
  1. Filter: blobs only, skip binary/excluded/too-large
  2. Priority files (always include)
  3. Coverage: 1–2 top-scored files per package root / top-level group
  4. Global fill by score until `MAX_FILES`

**Test:**
* Add `/debug/selection` endpoint
* Run on `psf/requests` and `expressjs/express`
  Expected: README + manifest present; src/ files included; no binaries, no lock files, no node_modules

**Why:** This is the core "repo processing" scoring differentiator — determines summary quality.
...
* Enforce budgets:
  ...
  * `README_MAX_CHARS = 12_000` head+tail
