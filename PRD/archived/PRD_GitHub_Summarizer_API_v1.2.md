Below is an updated **PRD v1.2** (rewritten to align tightly to the evaluation rubric + your “develop with OpenAI, swap to Nebius late” plan), followed by a **README skeleton** that the evaluators can follow verbatim.
...
Evaluators will test with **`NEBIUS_API_KEY`**. You want to develop with **OpenAI-compatible keys** first, then swap.

### Design

Implement a provider abstraction with this selection logic:

1. If `NEBIUS_API_KEY` exists → use **Nebius** (final path)
2. Else if `OPENAI_API_KEY` exists → use **OpenAI** (dev path)
3. Else → return 500 with message “No LLM API key configured”

**Why this works**

* You can develop locally with OpenAI immediately.
* You can “flip” to Nebius late by setting `NEBIUS_API_KEY` and ensuring the Nebius base URL/model config is correct.
* Evaluator environment will naturally use Nebius.

> Keep the request/response schema identical across providers to avoid differences in output parsing.
